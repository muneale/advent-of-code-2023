...
var numDotRe = regexp.MustCompile(`[0-9\.]`)
...
func (m *Matrix) GetEngineSchematicSum() int {
	...
	for i := 0; i < m.Rows; i++ {
		...
		for j := 0; j < m.Cols; j++ {
			if !numDotRe.MatchString(m.At(i, j)) {
...
func (m *Matrix) GetSchamticGearRatioSum() int {
	...
	for i := 0; i < m.Rows; i++ {
		...
		for j := 0; j < m.Cols; j++ {
			...
			// Found a gear
			if gearRe.MatchString(m.At(i, j)) {
				//fmt.Printf("Found a symbol '%s' at [%d,%d]\n", m.At(i, j), i, j)
				...
				// fmt.Printf("Found %d numbers: %v\n", len(gearNumbers), gearNumbers)
